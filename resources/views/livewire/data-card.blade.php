    ...
    <div>
        <div class="mx-auto w-max mb-4 py-2 relative">
            ...
            <div class="-mt-1 -mr-1 text-white font-bold z-20 absolute bottom-2 left-2">
                <form action="{{ route($destroy, $param) }}" method="POST">
                    @csrf
                    @method('put')
                    <label for="s{{$object->id}}"><span
                            class="fas {{ $object->is_active == 1 ? 'fa-trash text-red-400 hover:text-red-700' : 'fa-recycle text-green-400 hover:text-green-700' }} text-md cursor-pointer "
                            title="Borrar"></span></label for="submit">
                    <input type="submit" value="" class="hidden" name="submit" id="s{{$object->id}}"
                        onclick="confirm('{{ $confirm }}') || event.preventDefault()">
                </form>
            </div>
